...
def make_figs(x,y,BINS,outfile,score,gradients,histo=True):
    ...
    #TRYING TO PLOT SOME GRADIENT LINES HERE
    #for row in range(0,y.size,y.size/10):
    #    for col in range(0,y.size/2,y.size/10):
    #        vertices = 10*np.array([[row,col],[row+gradients[row,col,0],col+gradients[row,col,1]]])
    #        axScores.plot(vertices[:,0],vertices[:,1],'b-')
    #        axScores.plot(vertices[0,0],vertices[0,1],'k.')
    #        #axScores.plot(vertices[1,0],vertices[1,1],'kd')
    ...
    axScores.streamplot(X, Y, U, V,color='k',density=7.,linewidth = 0.3)#linewidth=lw)
...
if __name__ == "__main__":
    ...
    if opts.seed:
        ...
        N = 200 #number of pixels in a fake test image
        noise_size=1.0
        x = np.arange(N)
        ...
        spread1 = int(N/10 + rng.random()*N/50)  # length scale
        spread2 = int(2+2*rng.random())  # length scale
        spread3 = int(20+2*rng.random())  # length scale
        shape1 = 0.0*np.exp(-0.5*np.power((x-mid1)*1.0/spread1,2.0))
        shape2 = 5.0*np.exp(-0.5*np.power((x-mid2)*1.0/spread2,2.0))
        shape3 = 1.0*np.exp(-0.5*np.power((x-mid3)*1.0/spread3,2.0))
        # noise character of sources
        variance = noise_size*(1.0 - 0.5*shape1 + shape2) # source 3 has no variance effect
    ...
    for half_wd in range(1,max_wd,1): # wd is width of the window
        for col in range(max_spread):
            ...
            if ((hi - lo)>1) and (md >= lo) and (md <= hi):
                ...
                #m
                mde = md+eps
                wgtsM = np.exp(-(np.power((xb-mde),2.)/(2.*np.power(half_wd,2.))))
                nkM = np.sum(wgtsM*Cxk_slice,axis=1)
                SRC_termM = calc_full(nkM, alpha_SRC, sum_SRC, lg_sum_SRC, sum_lg_SRC)
                BG_termM = calc_full(nkM, alpha_BG, sum_BG, lg_sum_BG, sum_lg_BG)
                scoreM1 = SRC_termM - BG_termM
                ...
                SRC_termM = calc_full(nkM, alpha_SRC, sum_SRC, lg_sum_SRC, sum_lg_SRC)
                BG_termM = calc_full(nkM, alpha_BG, sum_BG, lg_sum_BG, sum_lg_BG)
                scoreM2 = SRC_termM - BG_termM

                numerical_gradm = (scoreM1 - scoreM2)/(2.*eps)
                #numerical_gradm = (scoreM1 - sc)/eps
                grad_ests[row,col,1] = numerical_gradm

                ratio = (scoreM1-scoreM2)/(2.*eps*gm)
                #diff = np.abs(numerical_gradm - gm)
                grad_diff[row,col,1] = np.abs(1.-ratio)
                #grad_diff[row,col,1] = diff
      

                #sigma
                se = half_wd+eps
                wgtsS = np.exp(-(np.power((xb-md),2.)/(2.*np.power(se,2.))))
                nkS = np.sum(wgtsS*Cxk_slice,axis=1)
                SRC_termS = calc_full(nkS, alpha_SRC, sum_SRC, lg_sum_SRC, sum_lg_SRC)
                BG_termS = calc_full(nkS, alpha_BG, sum_BG, lg_sum_BG, sum_lg_BG)
                scoreS1 = SRC_termS - BG_termS
                ...
                SRC_termS = calc_full(nkS, alpha_SRC, sum_SRC, lg_sum_SRC, sum_lg_SRC)
                BG_termS = calc_full(nkS, alpha_BG, sum_BG, lg_sum_BG, sum_lg_BG)
                scoreS2 = SRC_termS - BG_termS

                numerical_grads = (scoreS1 - scoreS2)/(2.*eps)
                #numerical_grads = (scoreS1 - sc)/eps
                grad_ests[row,col,0] = numerical_grads

                ratio = (scoreS1-scoreS2)/(2.*eps*gs)
                #diff = np.abs(numerical_grads - gs)
                grad_diff[row,col,0] = np.abs(1.-ratio)
                #grad_diff[row,col,0] = diff
    ...
    with file(gradientfile,'w') as out:
       ...
       for dslice in gradients:
           ...
           out.write('# New slice\n')
    gradientfile_est = outfile + '_gradients_ests.txt'
    with file(gradientfile_est,'w') as out:
       out.write('# Array shape: {0}\n'.format(grad_ests.shape))
       for dslice in grad_ests:
           np.savetxt(out,dslice)
           out.write('# New slice\n')
    gradientfile_diff = outfile + '_gradients_diff.txt'
    with file(gradientfile_diff,'w') as out:
       out.write('# Array shape: {0}\n'.format(grad_diff.shape))
       for dslice in grad_diff:
           np.savetxt(out,dslice)
           out.write('# New slice\n')


    print 'max grad diff: %s\n' % np.max(grad_diff)
    print 'min grad diff: %s\n' % np.min(grad_diff)
    print 'mean grad diff: %s\n' % np.mean(grad_diff)
    print 'grad diff < 1: %s\n' % ((np.sum(grad_diff<1.)/float(grad_diff.size))*100.)
    print 'grad diff < 0.1: %s\n' % ((np.sum(grad_diff<0.1)/float(grad_diff.size))*100.)
    print 'grad diff < 0.01: %s\n' % ((np.sum(grad_diff<0.01)/float(grad_diff.size))*100.)
    print 'grad diff < 0.001: %s\n' % ((np.sum(grad_diff<0.001)/float(grad_diff.size))*100.)
    print 'grad diff < 0.0001: %s\n' % ((np.sum(grad_diff<0.0001)/float(grad_diff.size))*100.)

    #X = np.arange(0,max_spread)
    #Y = np.arange(0,max_wd)
    #Y,X = np.mgrid[0:max_wd,0:max_spread]
    #U = gradients[:,:,1]
    #V = gradients[:,:,0]
    #plt.streamplot(X, Y, U, V,color='k',density=8.,linewidth=0.2)
    #gradfig = gradientfile.split('.')[0]
    #plt.savefig(gradfig)
    #plt.clf()

    #slopes = gradients[:,:,1]
    #plt.imshow(slopes,interpolation='nearest',origin='lower')
    #gradfigm = gradfig + '_mgrad'
    #plt.savefig(gradfigm)
    #plt.clf()
